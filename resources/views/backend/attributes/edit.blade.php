...
class OccupancyForecastReportForm extends Form{
	...
	function get_total_room_status($from_date,$to_date,$portal_id,$cond_confirm)
    {
		...
		$sql2='
				SELECT
						...
						rs.in_date,
                        date_to_unix(rs.in_date) as time6h_indate,
						rs.status,
                        rr.arrival_time,
	  ...
	  $room_in_outs = DB::fetch_all($sql2);
       /** kieu fix TH ngay 08-06-2016(báo cáo không khop v?i báo cáo chiem dung phong)) **/
        ...
        foreach($room_in_outs as $key=>$value)
		{
		     if(isset($reservation_virtual[$value['change_room_from_rr']])){
		      //System::debug($room_in_outs[$key]);
		      unset($room_in_outs[$key]);
		     }
        }
      /** END kieu fix TH ngay 08-06-2016(báo cáo không khop v?i báo cáo chiem dung phong)) **/
	  $room_arr = array();
	  $total_userday=0;
	  foreach($room_in_outs as $key=>$value)
      {
        ...
        if(1==1)
        {
         ...
         // end KimTan xu ly truong hop doi phong
             if($value['reservation_status']!='CANCEL' and $value['reservation_status']!='NOSHOW')
             {
                if($value['in_date']==$value['arrival_time'] && $value['time_in']>=$value['time6h_indate'] && $value['change_room_from_rr']==0)//phòng den kimtan ngay 07/07/15
                {
                ...
                if($value['change_room_from_rr'] != 0 and date('d/m/Y',$value['time_in'])!= date('d/m/Y',$value['time_old']) and $value['time_old'] > $value['time6h_indate'] and Date_Time::to_time(Date_Time::convert_orc_date_to_date($value['in_date'],'/'))<Date_Time::to_time(Date_Time::convert_orc_date_to_date($value['departure_time'],'/')) )
                {
                    if(!isset($room_occupancy[$value['in_date']]['total_arrival_room']))
                    {
							$room_occupancy[$value['in_date']]['total_arrival_room']= $value['total'];
    ...
    function get_ei_lo_li($date_from,$date_end,$portal_id,$cond_confirm,$type=false)
    {
        ...
        if (!empty($items))
        {
            ...
            {
                //KimTan21/01/15: neu la ei lo li thi khong can tra ve phong van tinh doanh thu va dem phong ei lo li. neu ko phai thi neu co tra ve phong se tinh doanh thu cho phong nhung ko dc dem so luong
	...
	function draw()
    {
        $from_day =(Url::get('from_day'))?Date_Time::to_orc_date(Url::get('from_day')):date('1/M/Y');
        $end_day = (Url::get('to_day'))?Date_Time::to_orc_date(Url::get('to_day')): Date_Time::to_orc_date(date(''.cal_days_in_month(CAL_GREGORIAN, date('m'), date('Y')).'/m/Y'));
        $portal_id =' AND 0 = 0';
        if(Url::get('portal_id'))
        {
            if(Url::get('portal_id') !='ALL')
        ...
        $time_from_day =(Url::get('from_day'))?Date_Time::to_time(Url::get('from_day')):(Date_Time::to_time(date('1/m/Y')));
        $time_to_day =(Url::get('to_day'))?Date_Time::to_time(Url::get('to_day')):(Date_Time::to_time(date(''.cal_days_in_month(CAL_GREGORIAN, date('m'), date('Y')).'/m/Y')));
